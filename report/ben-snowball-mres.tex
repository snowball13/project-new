...
Equations (13, 14) above are implemented in a code library \citep{merigotpymongeampere}, which is used in \cite{gallouet2016lagrangian}. 
...
\end{align}
so \(c\) is independent of \(t\) and gives the "distance" from an incompressible state that the particles initially lie. The initial condition \(m^0\) is found using Lloyd's algorithm, an iterative algorithm that randomly chooses a configuration of the particles in the domain and iterates to move them closer to an incompressible configuration \citep{merigot2017algorithm}. We then solve
\begin{align}
 &\begin{cases}
  \dot{m} = u \\
...
Upon using this algorithm on the Beltrami flow example, we found that it did not behave as desired if we increased the tolerance of the constraint, i.e. set \(c := 2 \; d^2_\Sb (m^0)\) (see Section 4: Results). The reason being that the system was being over-constrained, as the particles can actually get closer to incompressible than \(c\). In Algorithm 1, the \(b^n\) term can be noisy, and so we do not wish to nudge the particles in a noisy direction every timestep.
